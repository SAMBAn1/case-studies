# Case Studies

Standalone marketing microsites for individual product case studies, linked from [sambitxsamba.com](https://www.sambitxsamba.com).

Each case study lives in its own folder under `docs/` and is a self-contained static site (no build step) served via GitHub Pages.

| Case study | Path |
|---|---|
| The PM-as-builder AI-native workflow | [`docs/ai-native-pm-workflow`](docs/ai-native-pm-workflow) |
| Data-driven vs. rule-based collections worklist | [`docs/worklist-vision`](docs/worklist-vision) |
| B2B commerce portal — GTM to first 100 customers | [`docs/i95dev-gtm`](docs/i95dev-gtm) |
| Overclocking experimentation log | [`docs/overclock`](docs/overclock) |
